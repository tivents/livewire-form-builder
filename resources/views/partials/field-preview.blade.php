...
@elseif ($type === 'row')
    <div class="text-xs text-indigo-400 font-medium flex items-center gap-1.5">
        ...
        Row — {{ count($field['children'] ?? []) }} field(s)
    </div>
    @if (!empty($field['children']))
        <div class="flex gap-1 mt-1 flex-wrap">
            @foreach ($field['children'] as $child)
                <span class="inline-block bg-indigo-50 text-indigo-500 px-1.5 py-0.5 rounded text-[10px]">{{ $child['label'] ?? $child['type'] }}</span>
            @endforeach
        </div>
    @endif
