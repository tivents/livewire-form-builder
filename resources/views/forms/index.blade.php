...
<x-livewire-form-builder::layout title="Forms">
    <div class="max-w-5xl mx-auto py-8 px-4">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Forms</h1>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            @if ($forms->isEmpty())
                <div class="py-16 text-center text-gray-400">
                    <p class="text-sm">No forms yet. <a href="{{ route('livewire-form-builder.forms.create') }}" class="text-indigo-600 hover:underline">Create your first form →</a></p>
                </div>
            @else
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="text-center px-5 py-3 font-semibold text-gray-600">Fields</th>
                            <th class="text-center px-5 py-3 font-semibold text-gray-600">Submissions</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-4 text-center">
                                @if ($form->is_active)
                                    <span class="inline-block bg-green-100 text-green-700 text-xs font-medium px-2 py-0.5 rounded-full">Active</span>
                                @else
                                    <span class="inline-block bg-gray-100 text-gray-500 text-xs font-medium px-2 py-0.5 rounded-full">Inactive</span>
                                @endif
                            </td>
                            <td class="px-5 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('livewire-form-builder.forms.edit', $form) }}"
                                       class="text-xs px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors">Edit</a>
                                    <form action="{{ route('livewire-form-builder.forms.destroy', $form) }}" method="POST"
                                          onsubmit="return confirm('Delete this form?')">
                                        @csrf @method('DELETE')
                                        <button class="text-xs px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg transition-colors">Delete</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
